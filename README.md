...
**想直接看报告长什么样**：[`docs/eval/sample-report.md`](docs/eval/sample-report.md) 是一份真实输出（qwen-plus + extract-v2，2026-08-29）。配套说明见 [`docs/eval/README.md`](docs/eval/README.md) —— 里面写清楚了这些数字能用来做什么、不能用来做什么。
...
`src/lib/extract/persist.ts` · `src/lib/extract/persist-logic.ts` · `src/app/app/leads/extract-panel.tsx`

eval 框架做完之后，抽取管线有一段时间**只被评测脚本调用**，业务流程里用不上 —— 跑分很好看，但没有一条真实线索经过它。这一节是把它接进业务的部分。

### 落库时一律标记为未确认

模型输出写进 `qualifications` 表时 `verified_fields` 是空数组。`match/queries.ts` 靠这个字段区分三种数据质量：

| 状态 | 含义 | 能否用于对机构报备 |
|---|---|---|
| `verified` | 人工逐字段核对过 | 可以 |
| `extracted` | 模型抽的，没人看过 | **不可以** |
| `empty` | 没有值 | —— |

**这个区分不是洁癖。** 拿未核实的抽取值去跟机构报客户资质，报错了损害的是中介关系 —— 那是这门生意里最难重建的东西。所以「谁确认过」必须是一等公民，不能靠人记得。

### 复核界面必须显示原话

prompt 强制模型对每个字段输出**引用的客户原文**，复核界面把原话显示在字段旁边。

没有原文对照的复核只是「审阅」—— 人看一眼数字觉得合理就点通过，而模型幻觉出来的值往往恰恰看起来合理。**有原文对照才是「核对」，这道关卡才不是假的。**

### 不提供「取中间值」按钮

模型对模糊表述输出区间（`36~47`，`value` 为 null）。界面上**没有**一键填中位数的操作。

人只有两个选择：保留区间，或者打电话问清后填精确值。**一键填 41 是把编造包装成效率**，而这个系统存在的理由正是不编造。

### 每次抽取新增一行，不覆盖

客户第一通电话说「社保交了两年」，第三通说「其实中间断过」。覆盖写会抹掉这个演进过程，而**演进过程本身就是判断可信度的依据** —— 一个前后说法反复的客户和一个说法稳定的客户，风险不同。
...
| `api_error` | 是 | 重试 |
| `timeout` | 是 | 重试 |
| `parse_error` | 否 | 改 prompt |
| `schema_invalid` | 否 | 加容错（见第四节第 1 条） |

合并成一个「失败」状态就没法知道该重试还是该改代码。**失败时不写 `qualifications` 表 —— 没有数据比有假数据安全。**

### 一个 `??` 引发的设计决定

复核接口里「不传某个字段」和「传 `null`」必须是两件事：

- 不传 = 这次不动它
- 传 `null` = 人工判定客户没说过，**要清掉原值**

如果 `buildQualPatch` 写成 `patch.x ?? before.x`，这两种情况会塌成同一个结果 —— **人就没法清空模型编出来的值，而清空恰好是修幻觉的唯一手段。** 最新一轮 eval 报告里唯一的错误就是一个布尔字段幻觉出 `true`，这里写错的话，那个错误在界面上永远改不掉。

所以用 `in` 判断键是否存在，而不是 `??`。同一组测试还守住两个容易被 `??` 吃掉的值：`hasCarLoan: false`（「全款买的车」是明确信息，不是未提及）和 `creditInquiries3m: 0`（「没查过征信」是 0 次，与「没提征信」语义相反）。

### 为什么把纯逻辑拆成单独文件

`persist.ts` import 了 `@/db`，跑单测需要 `DATABASE_URL`；mock drizzle 的链式 builder 测的是「mock 像不像 drizzle」，不是业务逻辑。

所以字段合并、状态判定、变更 diff 这些纯函数拆进 `persist-logic.ts`，37 个用例不碰数据库。**不可测的代码通常是耦合没拆开。**

### 抽取面板默认折叠

点开才触发抽取。理由是这个动作**花钱 + 等 5-15 秒 + 需要逐字段核对** —— 它不是在车上单手能完成的操作，不该和「看一眼该打给谁」放在同一层。
...
`src/lib/leads/create.ts` · `POST /api/leads` · `src/app/app/leads/new-lead.tsx`

系统原先只有一条线索入口：官网表单 → `/api/ingest/leads`。但实际获客来自私域微信，**这些客户根本不会去官网填表** —— 他们直接在微信里问「我这条件能贷多少」。整个抽取和匹配管线对这批人是不可达的。

几个刻意的决定：

**① 只有称呼和手机号必填。** 必填项越多，人越倾向「等信息全了再录」，结果是根本不录。资质字段后面走抽取或手动补。

**② 姓名和手机号必须手填，刻意不交给模型抽。** 手机号是线索的唯一身份（`phone_hash` 唯一索引），抽错一位就是另一个人。**资质字段抽错有复核关卡兜着，身份字段抽错没有** —— 它会静默地创建一条属于别人的线索。

**③ 来源默认 `wechat` 不是 `manual`。** 这个入口九成用于微信直聊。`manual` 描述的是「录入方式」，`wechat` 描述的是「客户从哪来」—— 后者才是内容策略要看的数据。归因错了，投入就会错配。

**④ 手机号重复不报错**，返回已存在的线索并提示「已在库里（线索 #N）」。客户隔周再问一次是常态，报错会诱发重复尝试，最后要么建出重复线索要么放弃录入。同时处理了唯一索引的竞态。

### 一条业务规则写进了代码强制

**转为「养客中」必须填重捞时间，否则接口返 400。**
...
**已完成**：抽取管线 · eval 框架 · 官网线索入库 · **手工建线索** · **抽取落库与逐字段人工复核** · 线索工作台 · 标注工具 · 认证与字段加密

测试分布（104 个用例）：

| 文件 | 用例 | 守的约束 |
|---|---|---|
| `extract/persist-logic.test.ts` | 37 | 「不传」与「传 null」不塌成同一结果；`false` / `0` 不被吞 |
| `extract/validate.test.ts` | 25 | 交叉校验：数值与布尔一致、模糊值不得有精确 value |
| `crypto.test.ts` | 22 | 同明文两次加密结果必须不同；指纹恒定 |
| `match/engine.test.ts` | 20 | 三态判定；「信息不足」不等于「不满足」 |
